...
sequenceDiagram
  ...
  client->>server: GET <br /> https://studies.cs.helsinki.fi/exampleapp/notes
  activate server
  ...
  client->>server: GET <br /> https://studies.cs.helsinki.fi/exampleapp/main.js
  activate server
  ...
  activate server
  server-->>client: No response, only close the connection
  deactivate server

  Note right of client: After submiting the form, the page reloads with the new note
```
